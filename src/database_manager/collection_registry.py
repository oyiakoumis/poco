...
class CollectionRegistry:
    ...
    async def register_collection(self, definition: CollectionDefinition) -> None:
        """
        Register a new collection definition.
        """
        dict_data = await definition.to_dict()
        await self._mongo_collection.insert_one(dict_data)
        logger.info("Registered collection definition for '%s'.", definition.name)

    async def get_collection_definition(self, collection_name: str) -> Optional[CollectionDefinition]:
        """
        Retrieve the definition for a specific collection.
        """
        data = await self._mongo_collection.find_one({"name": collection_name})
        if data:
            return CollectionDefinition.from_dict(data, self)
        logger.warning("Collection definition for '%s' not found.", collection_name)
        return None

    async def list_collection_definitions(self) -> List[CollectionDefinition]:
        """
        List all registered collection definitions.
        """
        cursor = self._mongo_collection.find()
        data = await cursor.to_list(None)
        return [CollectionDefinition.from_dict(doc, self) for doc in data]

    async def search_similar_collections(
        ...
        """
        embedding = await definition.embedding
        ...
        cursor = self._mongo_collection.aggregate(pipeline)
        results = await cursor.to_list(None)
        return [CollectionDefinition.from_dict(data, self) for data in results]

    async def update_collection_definition(self, definition: CollectionDefinition) -> None:
        """
        Update an existing collection definition.
        """
        definition.updated_at = datetime.now(timezone.utc)
        dict_data = await definition.to_dict()
        await self._mongo_collection.update_one({"name": definition.name}, {"$set": dict_data})
    ...
    async def unregister_collection(self, collection_name: str) -> None:
        ...
        """
        await self._mongo_collection.delete_one({"name": collection_name})
