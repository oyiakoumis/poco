        ...
        try:
            ...
            async for event in graph.astream({"messages": messages}, config, stream_mode="updates"):
                ...
                if delta:
                    ...
                    yield StreamingService._format_sse_event(ChatResponse(delta=delta, done=False))

            # Store the complete response
            await StreamingService._store_response(
                conversation_db, user_id, conversation_id, full_response or "I apologize, but I couldn't process your request."
            )

            # Send the final event with the complete message for backward compatibility
            yield StreamingService._format_sse_event(
                ChatResponse(
                    delta="",
                    done=True,
            ...
            yield StreamingService._format_sse_event(
                ChatResponse(error=str(e), done=True, message=f"Error: {str(e)}")  # Include error message for backward compatibility
            )
            ...
            if not full_response and not error_occurred:
                ...
                await StreamingService._store_response(conversation_db, user_id, conversation_id, fallback_message)

                # Send the fallback message and a "done" event with the complete message
                yield StreamingService._format_sse_event(ChatResponse(delta=fallback_message, done=False))
                yield StreamingService._format_sse_event(
                    ChatResponse(
                        delta="",
                        done=True,
