...
class SchemaField(BaseModel):
    ...
    field_name: str = Field(description="Name of the field in the dataset schema", min_length=1, max_length=128, examples=["user_age"])
    description: str = Field(
        description="Detailed description of what this field represents", min_length=1, max_length=500, examples=["Age of the user in years"]
    )
    type: FieldType = Field(description="Data type of the field", examples=[FieldType.INTEGER])
    required: bool = Field(default=False, description="Whether this field must be present in all records", examples=[True])
    unique: bool = Field(default=False, description="Whether this field's values must be unique across all records", examples=[True])
    default: Optional[Any] = Field(default=None, description="Default value for the field", examples=[0])
    options: Optional[List[str]] = Field(default=None, description="List of allowed values for select/multi-select fields", examples=[["option1", "option2"]])
