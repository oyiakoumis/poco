...
class TestManagerSetup:
    ...
    async def test_setup_creates_indexes(self, mock_client: AsyncMock, mock_db: AsyncMock) -> None:
        ...
        # Configure create_indexes to return new awaitable each time
        datasets_collection.create_indexes.side_effect = lambda x: AsyncMock(return_value=[])()
        records_collection.create_indexes.side_effect = lambda x: AsyncMock(return_value=[])()
...
class TestDatasetOperations:
    ...
    async def test_create_dataset(
        self, manager: DatasetManager, user_id: str, dataset_id: str, sample_structure: List[Field]
    ) -> None:
    ...
    async def test_create_dataset_duplicate_name(
        ...
        # Setup - override manager's mock behavior
        manager._datasets.insert_one.side_effect = lambda *args, **kwargs: AsyncMock(side_effect=Exception("duplicate key error"))()
    ...
    async def test_update_dataset(
        self, manager: DatasetManager, user_id: str, dataset_id: str, sample_dataset: Dataset, sample_structure: List[Field]
    ) -> None:
...
class TestRecordOperations:
    ...
    async def test_create_record(
        self, manager: DatasetManager, user_id: str, dataset_id: str, record_id: str
    ) -> None:
    ...
    async def test_get_record(
        self, manager: DatasetManager, user_id: str, dataset_id: str, record_id: str, sample_record: Record
    ) -> None:
